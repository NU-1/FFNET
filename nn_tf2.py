    ...
    def __init__(self,layers,learning_rate_,loss_type,opt_type):
        ...
        for i in layers[:-1]:
            self.hidden_layers.append(tf.keras.layers.Dense(
                layers[i].num_output, activation=layers[i].activation_type, kernel_initializer='RandomNormal'))

        self.output_layer = tf.keras.layers.Dense(
            layers[-1].num_output , activation=layers[-1].activation_type, kernel_initializer='RandomNormal')
